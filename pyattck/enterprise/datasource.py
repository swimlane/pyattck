...
class AttckDataSource(AttckObject):
    ...
    """Enterprise MITRE ATT&CK Data Source object
    ...
    def __init__(self, attck_obj = None, _data_component_filter=None, **kwargs):
        """
                    ...
                    if data_sources.get(self.name):
                        for component in self.data_components:
                            if  component.name in data_sources[self.name]:
                                return_list.append(AttckTechnique(attck_obj=self.__attck_obj, **item))
        return return_list
